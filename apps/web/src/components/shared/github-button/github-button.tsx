...
function GithubButton({
  ...
  // Fetch GitHub stars (if repoUrl is provided)
  const githubStarsQuery = useGithubStars(repoUrl);
  const fetchedStars = githubStarsQuery?.data;

  // Determine the effective target for animation and width reservation
  const effectiveTargetStars = React.useMemo(() => {
    if (typeof targetStars === 'number' && targetStars > 0) return targetStars;
    if (typeof fetchedStars === 'number' && fetchedStars > 0)
      return fetchedStars;
    return 0;
  }, [targetStars, fetchedStars]);
  ...
  // Reset animation state when inputs or effective target change
  useEffect(() => {
    setHasAnimated(false);
    setStarProgress(filled ? 100 : 0);
    if (!autoAnimate && !useInViewTrigger) {
      // Static display: show the target immediately when available
      setCurrentStars(
        effectiveTargetStars > 0 ? effectiveTargetStars : initialStars,
      );
    } else {
      // Animated display: keep initial number until animation triggers
  ...
  // Auto-start animation or use in-view trigger (only when a target is available)
  useEffect(() => {
    if (effectiveTargetStars <= 0) return;
    if (useInViewTrigger) {
      ...
      >
        ...
        {fixedWidth && (
          <span className="opacity-0 h-0 overflow-hidden tabular-nums">
            {formatNumber(effectiveTargetStars || targetStars || initialStars)}
          </span>
        )}
      </div>
    </button>
...
// React Query hook to fetch GitHub stargazers count for a repo
function useGithubStars(repoUrl?: string) {
  // Accept either a full GitHub URL or an "owner/repo" string
  const repoPath = React.useMemo(() => {
  ...
  return useQuery<number | undefined>({
    ...
    // A small retry helps in case of transient network issues
    retry: 1,
  });
}
